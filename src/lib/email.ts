...
/**
 * 7. Verification OTP Email (6-digit code, zero magic links)
 */
export async function sendVerificationOtpEmail({
  ...
  token: string;
  name?: string;
  purpose?: 'registration' | 'login' | 'rsvp';
}) {
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Your Vibe Verification Code</title>
  <style>
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #F8FAFC; margin: 0; padding: 32px 12px; color: #0F172A; }
    .email-wrapper { max-width: 560px; margin: 0 auto; background: #FFFFFF; border-radius: 20px; overflow: hidden; border: 1px solid #E2E8F0; box-shadow: 0 10px 25px -5px rgba(15, 23, 42, 0.05); }
    .brand-accent-strip { height: 5px; background: linear-gradient(90deg, #0F172A 0%, #E8621A 100%); width: 100%; }
    .header { padding: 32px 36px 20px 36px; border-bottom: 1px solid #F1F5F9; }
    .brand-title { font-size: 22px; font-weight: 900; color: #0F172A; letter-spacing: -0.5px; margin: 0; line-height: 1; }
    .brand-badge { color: #E8621A; font-size: 11px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; margin-left: 6px; }
    .brand-sub { font-size: 11px; color: #64748B; margin-top: 5px; display: block; font-weight: 500; }
    .body-content { padding: 36px; }
    .welcome-heading { font-size: 22px; font-weight: 800; color: #0F172A; margin: 0 0 12px 0; }
    .lead-text { font-size: 14px; line-height: 1.6; color: #475569; margin: 0 0 24px 0; }
    .otp-card { background: #0F172A; border-radius: 16px; padding: 28px 24px; text-align: center; margin: 28px 0; color: #FFFFFF; }
    .otp-label { font-size: 10px; font-weight: 800; letter-spacing: 2px; text-transform: uppercase; color: #94A3B8; margin-bottom: 10px; }
    .otp-code { font-family: -apple-system, SFMono-Regular, Consolas, monospace; font-size: 40px; font-weight: 900; letter-spacing: 10px; color: #FFFFFF; margin: 0; padding-left: 10px; }
    .otp-meta { font-size: 11px; color: #94A3B8; margin-top: 12px; }
    .instruction-box { background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 12px; padding: 16px 20px; margin: 24px 0; font-size: 13px; color: #334155; line-height: 1.5; }
    .security-note { border-top: 1px solid #F1F5F9; padding-top: 20px; margin-top: 28px; font-size: 12px; color: #94A3B8; line-height: 1.5; }
    .footer { padding: 24px 36px; background: #F8FAFC; border-top: 1px solid #E2E8F0; text-align: center; font-size: 11px; color: #64748B; }
  </style>
</head>
<body>
  <div class="email-wrapper">
    ...
    <div class="body-content">
      ...
      <p class="lead-text">${name ? `Hi <strong>${name}</strong>, ` : ''}${subtext}</p>
      <div class="otp-card">
      ...
      <div class="security-note">
        ...
        <p style="margin: 0;">Never share this code with anyone. If you did not initiate this request, you can safely ignore this email.</p>
      </div>
    </div>
    <div class="footer">
      <div><strong>Vibe by Swaniki</strong> · India-First White-Label Experience Platform</div>
      <div style="margin-top: 4px; color: #94A3B8;">Secure automated system notification · Asia/Kolkata (IST)</div>
    </div>
  </div>
</body>
</html>
