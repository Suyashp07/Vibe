      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md bg-surface rounded-2xl border border-border shadow-elevated p-6 sm:p-8 animate-in fade-in">
          ...
          <div className="mb-6">
            ...
            <h1 className="font-sans font-bold text-2xl text-ink">Create your account</h1>
            <p className="text-xs text-ink-muted mt-1">
              Join Vibe to host events or RSVP to curated experiences near you.
            </p>
          </div>
          ...
          <form onSubmit={handlePasswordSignUp} className="space-y-3.5">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  required
                />
                <User className="w-4 h-4 text-ink-muted absolute right-3.5 top-3 pointer-events-none" />
              </div>
            </div>
            ...
            {role === 'organizer' && (
              <div>
                <label className="block text-xs font-semibold text-ink mb-1.5">
                  Host Profile Handle <span className="text-ink-muted font-normal">(vibe.events/@handle)</span>
                </label>
                <div className="relative">
                  <span className="absolute left-3.5 top-2.5 text-xs font-mono text-ink-muted">@</span>
                  <input
                    type="text"
                    value={handle}
                    onChange={(e) => setHandle(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))}
                    placeholder="mybrand"
                    className="w-full pl-8 pr-3.5 py-2.5 text-xs bg-surface border border-border rounded-xl focus:outline-none focus:border-ink focus:ring-1 focus:ring-ink transition-colors text-ink font-mono placeholder:text-ink-muted"
                  />
                </div>
              </div>
            )}
            ...
            <div>
              <label className="block text-xs font-semibold text-ink mb-1.5">
                Password <span className="text-ink-muted font-normal">(min. 6 characters)</span>
              </label>
              <div className="relative">
                <input
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-brand hover:bg-brand-mid text-white text-xs font-bold rounded-xl shadow-xs transition-colors disabled:opacity-50 flex items-center justify-center gap-2 mt-2"
            >
              {loading ? (
          ...
          <div className="mt-6 pt-4 border-t border-border text-center text-xs text-ink-muted">
            ...
            <Link
              href={`/login?role=${role}${redirectParam ? `&redirect=${encodeURIComponent(redirectParam)}` : ''}`}
