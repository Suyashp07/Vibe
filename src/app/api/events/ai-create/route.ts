...
export async function POST(req: NextRequest) {
  try {
    ...
    const insertPayload = {
      ...
      is_public: surety.autoApproved && isPublic !== false,
      is_private: !surety.autoApproved || isPublic === false,
      visibility: (!surety.autoApproved || isPublic === false) ? 'private' : 'public',
      status: surety.autoApproved ? 'live' : 'draft',
      confidence_score: suretyScore / 100,
      ai_generated: true,
