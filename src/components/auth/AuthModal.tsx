...
export default function AuthModal({
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      // Auto-reset loading state after 8 seconds if redirect happens in background
      setTimeout(() => {
        setGoogleLoading(false);
      }, 8000);
    } catch (err: any) {
      ...
      <div className="relative w-full max-w-sm bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 z-10 animate-in zoom-in-95 duration-150">
        ...
        <form onSubmit={handleEmailAuth} className="space-y-3">
          ...
          <button
            ...
            className="w-full py-2.5 bg-[#0A0A0A] hover:bg-gray-800 text-white text-xs font-bold rounded-xl shadow-xs transition-colors disabled:opacity-50 flex items-center justify-center gap-2 mt-2 cursor-pointer"
          >
            {emailLoading ? (
        ...
        <button
          ...
          disabled={emailLoading || googleLoading}
          type="button"
          className="w-full py-2.5 border border-gray-300 hover:bg-gray-50 text-gray-800 text-xs font-bold rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center gap-2.5 shadow-xs cursor-pointer"
        >
          {googleLoading ? (
